...
let attentionScore (query:Vector) (keyLookup:int->float) : float =    
    query
        |> elementWiseMultiply ([|0..query.Length - 1|] |> Array.Parallel.map keyLookup)
        |> Array.sum
...
let weightedAttention (attention: Vector) (valueLookup:int->float) : float =
    attention
        |> elementWiseMultiply ([|0..attention.Length - 1|] |> Array.Parallel.map valueLookup)
        |> Array.sum
...
let attentionForOneHead (keyLookup:int->int->float) (valueLookup:int->int->float) (tokenPosition:int) (query: Vector): Vector =
    let attention =
        [|0..tokenPosition|]
            |> Array.Parallel.map (fun x -> attentionScore query (keyLookup x))
            |> softMax

    [|0..query.Length - 1|]
        |> Array.Parallel.map (fun i -> (weightedAttention attention (fun j -> valueLookup j i))) // weird usage of value lookup function because of the order of operations would prefer to change weightedAttention, but that would fail test cases
...
let attention  (keyLookup:int->int->int->float) (valueLookup:int->int->int->float) (tokenPosition:int) (query: MultiHead) : Vector =
    query
        |> Array.Parallel.mapi (fun i q -> (attentionForOneHead (keyLookup i) (valueLookup i) tokenPosition q))
